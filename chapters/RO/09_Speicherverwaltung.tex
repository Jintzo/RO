...
\paragraph{Virtueller Speicher --- Verwaltung (Segmentierung)}
\begin{items}
...
\paragraph{Segmentbasierte Speicherverwaltung --- Implementierung}
\begin{items}
...
\paragraph{Probleme --- Einlagerungszeitpunkt}
\begin{items}
...
\paragraph{Probleme --- Ersetzungsproblem (Segmentierung)}
\begin{items}
...
\begin{items}
  ...
  \item Segment-Register: \textbf{Code-Segment} (\code{CS}), \textbf{Stack-Segment} (\code{SS}), \textbf{Daten-Segment} (\code{DS}), \textbf{Extra-Segment} (\code{ES}, \code{FS}, \code{GS},...) \\*
...
\paragraph{Adressierungsmodi --- Beispiel x86}
\begin{items}
...
\begin{items}
  ...
  \begin{enumeration}
    ...
    \item vier Schutzebenen (Segmentverwaltung): \\*
      - \code{PL0} (am vertrauenswürdigsten) bis \code{PL3}
  \end{enumeration}
\end{items}
